...
def normalize_node(element: HtmlElement):
    # 规范化节点（清除无用节点）
    etree.strip_elements(element, *USELESS_TAG)  # 清除无用节点，['style', 'script', 'link', 'video', 'iframe', 'source', 'picture', 'header', 'blockquote']
...
def pre_parse(html):
    # 剔除换行标签
    html = re.sub('</?br.*?>', '', html)
    element = fromstring(html)  # 将html解析成 HtmlElement对象
...
def pad_host_for_images(host, url):
    ...
    :return:
    """
    if url.startswith('http'):  # http开头的其实也把https包含在内了
        return url
